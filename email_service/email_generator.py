...
@task
def generate_yearly_report_html(
    yearly_data: Sequence[Row[Tuple[int, str, int, Any]]],
    year: int,
) -> str:
    table_rows = ""
    for month, month_name, abs_count, abs_percentage in yearly_data:
        table_rows += f"""
        <tr>
            <td>{month}</td>
            <td>{month_name}</td>
    ...
    <head>
        <meta charset="UTF-8">
        <title>Rapport de l'année {year}</title>
        <style>
            body {{
                font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
                background-color: #f9fafb;
    ...
    <body>
        <div class="container">
            <h2>Rapport des absences {year}</h2>
            <table>
                <thead>
                    <tr>
                        <th>Mois</th>
