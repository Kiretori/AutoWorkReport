...
@task
def fetch_employees_per_date(target_date_id: int) -> Sequence[Row[Tuple[DimEmployee, Any, bool]]] | None:
    ...
    with Session(engine) as session:
        ...
        employees = session.execute(
            select(DimEmployee, work_duration.label("work_duration"), DailyAttendance.present)
...
def fetch_absent_employees(employees_data: Sequence[Row[Tuple[DimEmployee, Any, bool]]]) -> List[DimEmployee]:
    absent_employees = []
...
def fetch_employees_under_working(employees_data: Sequence[Row[Tuple[DimEmployee, Any, bool]]], under_work_threshold: float) -> List[Tuple[DimEmployee, Any]]:
