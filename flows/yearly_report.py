...
@flow(flow_run_name=generate_yearly_flow_name)
def yearly_report(target_year: int | None = None):
    ...
    yearly_data = fetch_yearly_data(target_year)
    logger.info("Generating Excel sheets")
    xlsx_filepath = generate_yearly_excel(
        target_year,
        f"yearly_report_{target_year}",
    )

    html_report = generate_yearly_report_html(yearly_data, target_year)
